...
When collecting or analyzing data, we frequently encounter groupings or nested structures underpinning that data. For example, we might have multiple measurements from the same individual or species, species co-occurrences across multiple temporal windows or localities, or 
...
We want inferences about one state to possibly inform inferences about another. This does not mean treating all states as identical. Instead we want to understand the distribution of states as well as those individual states. For example, this would mean the estimating the average and spread among the trophic levels as well as the individual values for each trophic level. Simultaneously inference means we can transfer information between states and improve our inferences.
