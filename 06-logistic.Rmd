...
sim_results <- tibble(sim = 1:4,
                      prob = draw,
                      coil_sim = map(draw, ~ ifelse(.x > 0.5, 1, 0)),
                      coil_state = map(1:4, ~ foram_ready$dextral)) %>% 
                 unnest()

sim_results %>%
  ggplot() +
  geom_jitter(aes(x = coil_sim, y = coil_state), alpha = 0.1) +
  facet_wrap(~ sim)
...
ppd <- (foram_design %*% t(reg_post[1:4, ])) %>%  # only first 4 draws
  purrr::compose(as.list, as.data.frame)(.) %>%
  map(., ~ invlogit(.x))

sim_results <- tibble(sim = 1:4,
                      prob = ppd,
                      coil_sim = map(ppd, ~ ifelse(.x > 0.5, 1, 0)),
                      coil_state = map(1:4, ~ foram_ready$dextral)) %>% 
                 unnest()
sim_results %>%
  ggplot() +
...
We've explored our variables. We've written out a model. We've explored the implications of our priors. Now let's fit our model to our data!
