...
Let's begin our population density model in a similar fashion to our previous one: an intercept-only model. Once this simplest model is established, we can then add our first continuous predictor from there.
...
Here is a quick visualization of the Cauchy distribution's behavior as you vary the scale parameter. The half-Cauchy is just this distribution reflected about the origin.
```{r cauchy_demo}
